...
class ModelSaleAffiliate extends Model {
	...
	public function getAffiliateByEmail(string $email) {
		$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "affiliate` WHERE LCASE(email) = '" . $this->db->escape(mb_strtolower((string)$email, 'UTF-8')) . "'");
	...
	public function addTransaction(int $affiliate_id, $description = '', $amount = '', int $order_id = 0) {
		...
		if ($affiliate_info) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "affiliate_transaction` SET affiliate_id = '" . (int)$affiliate_id . "', order_id = '" . (int)$order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float)$amount . "', date_added = NOW()");
