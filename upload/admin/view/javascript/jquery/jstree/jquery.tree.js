	...
	function tree_component() {
		return {
			...
			init : function(elem, conf) {
				...
				if (this.settings.languages && this.settings.languages.length) {
					...
					for (var ln = 0; ln < this.settings.languages.length; ln++) {
						...
						if (st !== false) {
							st.style.display = (this.settings.languages[ln] == this.current_lang) ? "" : "none";
						}
					}
				} else {
				...
				for (var t in this.settings.types) {
					...
					if (this.settings.types[t].icon.image || this.settings.types[t].icon.position) {
						if (t == "default") {
							type_icons += "#" + this.container.attr("id") + " li > a ins { ";
						} else {
							type_icons += "#" + this.container.attr("id") + " li[rel=" + t + "] > a ins { ";
						}
						if (this.settings.types[t].icon.image) {
							type_icons += " background-image:url(" + this.settings.types[t].icon.image + "); ";
						}
						if (this.settings.types[t].icon.position) {
							type_icons += " background-position:" + this.settings.types[t].icon.position + "; ";
						}
						type_icons += "} ";
					}
				}
			...
			attach_events : function() {
				...
				$("#" + this.container.attr("id") + " li").live("click", function(event) { // WHEN CLICK IS ON THE ARROW
				...
				$("#" + this.container.attr("id") + " li a").live("click.jstree", function(event) { // WHEN CLICK IS ON THE TEXT OR ICON
					...
					if (_this.settings.rules.multiple != false && _this.selected_arr.length > 1 && obj.children("a:eq(0)").hasClass("clicked")) {
						...
						for (var i in _this.selected_arr) {
							if (!_this.selected_arr.hasOwnProperty(i)) {
								continue;
							}
						...
						if (counter > 0) {
							...
							if (_this.settings.languages.length > 0) {
								tmp.find("a").not("." + _this.current_lang).hide();
							}
			...
			scroll_check : function(x,y) {
				...
				if (y - off.top < 20) {
					cnt.scrollTop(Math.max( (st - _this.settings.ui.scroll_spd) ,0));	// NEAR TOP
				}
				if (cnt.height() - (y - off.top) < h_cor) {
					cnt.scrollTop(st + _this.settings.ui.scroll_spd);							// NEAR BOTTOM
				}
				if (x - off.left < 20) {
					cnt.scrollLeft(Math.max( (sl - _this.settings.ui.scroll_spd),0));		// NEAR LEFT
				}
				if (cnt.width() - (x - off.left) < 40) {
					cnt.scrollLeft(sl + _this.settings.ui.scroll_spd);							// NEAR RIGHT
				}
			...
			check : function(rule, obj) {
				...
				if (obj === -1) {
					...
					if (typeof this.settings.types[t] != "undefined" && typeof this.settings.types[t][rule] != "undefined") {
						v = this.settings.types[t][rule];
					} else if (typeof this.settings.types["default"] != "undefined" && typeof this.settings.types["default"][rule] != "undefined") {
			...
			check_move : function(nod, ref_node, how) {
				...
				if (tree1 && tree1 != tree2) {
					...
					if (m == "none" || ($.isArray(m) && $.inArray(tree1.id, m) == -1)) {
						return false;
					}
				}
				...
				if (typeof r.valid_children != "undefined" && (r.valid_children == "none" || (typeof r.valid_children == "object" && $.inArray(nod_type, $.makeArray(r.valid_children)) == -1))) {
					return false;
				}
				if (this.settings.rules.use_max_children) {
					if (typeof r.max_children != "undefined" && r.max_children != -1) {
						...
						if (r.max_children < p.find('> ul > li').size() + c_count) {
							return false;
						}
					}
				}
			...
			create : function(obj, ref_node, position) {
				...
				if (!obj.attributes[this.settings.rules.type_attr]) {
					obj.attributes[this.settings.rules.type_attr] = this.get_type(tmp) || "default";
				}
				if (this.settings.languages.length) {
					if (!obj.data) {
						obj.data = {}; torename = true;
					}
					...
					if (!obj.data) {
						obj.data = this.settings.lang.new_node; torename = true;
					}
				}
				...
				if (typeof r.valid_children != "undefined" && (r.valid_children == "none" || ($.isArray(r.valid_children) && $.inArray(nod_type, r.valid_children) == -1))) {
					return this.error("CREATE: NODE NOT A VALID CHILD");
				}
				if (this.settings.rules.use_max_children) {
					if (typeof r.max_children != "undefined" && r.max_children != -1 && r.max_children >= this.children(ref_node).size()) {
			...
			rename : function(obj, new_name) {
				...
				if (!this.callback("beforerename", [obj.get(0), _this.current_lang, _this])) {
				...
				if ((typeof new_name).toLowerCase() == "string") {
					...
					obj.contents().each(function() {
						if (this.nodeType == 3) {
							last_value = this.data; return false;
						}
					});
					...
					_this.inp
						...
						.bind("keyup", function(event) {
							...
							if (key == 27) {
								this.value = last_value; this.blur();
								return;
							}
							if (key == 13) {
								this.blur();
			...
			remove : function(obj) {
				...
				if (obj && (!this.selected || this.get_node(obj).get(0) != this.selected.get(0))) {
					...
					if (obj.size()) {
						...
						if (!this.callback("beforedelete",[obj.get(0), _this])) {
						...
						if (obj.find("a.clicked").size()) {
							...
							_this.selected_arr = [];
							this.container.find("a.clicked").filter(":first-child").parent().each(function() {
								if (!reset_selected && this == _this.selected.get(0)) {
									reset_selected = true;
								}
					...
					if (!this.callback("beforedelete",[this.selected.get(0), _this])) {
						return this.error("DELETE: STOPPED BY USER");
					}
			...
			prev : function(obj, strict) {
				...
				if (strict) {
					return (obj.prevAll("li").size() > 0) ? obj.prevAll("li:eq(0)") : false;
				}
			...
			moved : function(what, where, how, is_new, is_copy, rb) {
				...
				if (is_new) {
					if (!this.callback("beforecreate", [this.get_node(what).get(0), this.get_node(where).get(0),how,this])) {
						return false;
					}
				} else {
					if (!this.callback("beforemove", [this.get_node(what).get(0), this.get_node(where).get(0),how,this])) {
						return false;
					}
				}
							...
							if (this.settings.languages.length == 0) {
								...
								for (var i in this.settings.languages) {
									...
									for (var j in tmp.settings.languages) {
										if (!tmp.settings.languages.hasOwnProperty(j)) {
											continue;
										}
