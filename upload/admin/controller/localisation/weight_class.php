<?php
/**
 * Class ControllerLocalisationWeightClass
 *
 * @package NivoCart
 */
class ControllerLocalisationWeightClass extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['weight_class_description'])) {
			...
			$this->data['weight_class_description'] = [];
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['weight_class_description'] as $language_id => $value) {
			if ((mb_strlen($value['title'], 'UTF-8') < 3) || (mb_strlen($value['title'], 'UTF-8') > 32)) {
				$this->error['title'][$language_id] = $this->language->get('error_title');
			}

			if (!$value['unit'] || (mb_strlen($value['unit'], 'UTF-8') > 4)) {
