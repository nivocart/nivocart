...
class ControllerModificationVQmods extends Controller {
	...
	public function __construct($registry) {
		...
		$this->vqmod_nivocart_script = substr_replace(DIR_SYSTEM, '/vqmod/xml/vqmod_nivocart.xml', -8);
	...
	public function index() {
		...
		$this->data['error_nivocart_version'] = $this->language->get('error_nivocart_version');
		$this->data['error_nivocart_xml'] = $this->language->get('error_nivocart_xml');
		$this->data['error_nivocart_xml_disabled'] = $this->language->get('error_nivocart_xml_disabled');
		$this->data['error_nivocart_xml_version'] = $this->language->get('error_nivocart_xml_version');
		...
		$this->data['error_vqmod_nivocart_integration'] = $this->language->get('error_vqmod_nivocart_integration');
	...
	protected function vqmodInstallationCheck() {
		...
		// Check that vqmod_nivocart.xml exists
		if (!is_file($this->vqmod_nivocart_script)) {
			if (is_file($this->vqmod_nivocart_script . '_')) {
				$this->session->data['error'] = $this->language->get('error_nivocart_xml_disabled');
			} else {
				$this->session->data['vqmod_installation_error'] = $this->language->get('error_nivocart_xml');
				return false;
			}
		}

		// Check that NivoCart 1.0.0 is being used
		if (!defined('VERSION') || version_compare(VERSION, '0.9.9', '<')) {
			$this->session->data['vqmod_installation_error'] = $this->language->get('error_nivocart_version');
			return false;
		}
		...
		if (version_compare(VERSION, '1.0.0', '>=')) {
			...
			if ((isset($xml->vqmver)) && (strtolower($xml->vqmver) != '2.x') && (version_compare($xml->vqmver, '2.1.7', '<'))) {
				$this->session->data['vqmod_installation_error'] = $this->language->get('error_nivocart_xml_version');
				return false;
			}
		}

		// Check if VQMod class is added to NivoCart
		if (!class_exists('VQMod')) {
			if (is_file($this->vqmod_dir . 'install/index.php') && is_file($this->vqmod_dir . 'install/ugrsr.class.php')) {
				...
				$this->session->data['vqmod_installation_error'] = $this->language->get('error_vqmod_nivocart_integration');
			}
			return false;
		}
		...
		// Check if vqcache files from vqmod_nivocart.xml have been generated
		$vqcache_files = array(
		...
		foreach ($vqcache_files as $vqcache_file) {
			// Only return false if vqmod_nivocart.xml_ isn't present (in case the user has disabled it) so they aren't locked out of VQMM
			if (!is_file($this->vqcache_dir . $vqcache_file) && !is_file($this->vqmod_nivocart_script . '_')) {
