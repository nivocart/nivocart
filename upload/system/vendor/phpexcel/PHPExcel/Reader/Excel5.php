...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader {
	...
	public function listWorksheetNames($pFilename) {
		...
		$this->_dataSize = strlen((string) $this->_data);
	...
	public function load($pFilename) {
			...
			while ($this->_pos <= $this->_dataSize - 4) {
				...
				switch ($code) {
					...
					case self::XLS_Type_NOTE: $this->_readNote();
						break;
						//case self::XLS_Type_IMDATA: $this->_readImData();
						break;
		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				if (count($explodes) == 2) {
					if (($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) ||
						($docSheet = $this->_phpExcel->getSheetByName(trim((string) $explodes[0], "'")))) {
						$extractedRange = $explodes[1];
						$extractedRange = str_replace('$', '', $extractedRange);
	...
	private function _readRecordData($data, $pos, $len) {
		...
		if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
			...
			while ($block != $endBlock) {
				...
				$recordData .= $this->_rc4Key->RC4(substr((string) $data, 0, $step));
			...
			$recordData .= $this->_rc4Key->RC4(substr((string) $data, 0, $len));
	 ...
	 * @param string $pFilename
	 */
	private function _loadOLE($pFilename): void {
		// OLE reader
		$ole = new PHPExcel_Shared_OLERead();
	 ...
	 */
	private function _readSummaryInformation(): void {
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = substr((string) $this->_summaryInformation, $secOffset + 8 + $offset, $byteLength);
				...
				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = PHPExcel_Shared_OLE::OLE2LocalDate(substr((string) $this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
	 ...
	 */
	private function _readDocumentSummaryInformation(): void {
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E:	//	null-terminated string prepended by dword string length
					...
					$value = substr((string) $this->_documentSummaryInformation, $secOffset + 8 + $offset, $byteLength);
					$value = PHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim((string) $value);
					break;
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = PHPExcel_Shared_OLE::OLE2LocalDate(substr((string) $this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
	 ...
	 */
	private function _readDefault(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		//		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readNote(): void {
		...
		$cellAddress = $this->_readBIFF8CellAddress(substr((string) $recordData, 0, 4));

		if ($this->_version == self::XLS_BIFF8) {
			$noteObjID = self::_GetInt2d($recordData, 6);
			$noteAuthor = self::_readUnicodeStringLong(substr((string) $recordData, 8));
...
//					->setAuthor( $author )
	 ...
	 */
	private function _readTextObject(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read BOF
	 */
	private function _readBof(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = substr((string) $this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readFilepass(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		...
		if (!$this->_verifyPassword(
			'VelvetSweatshop',
			substr((string) $recordData, 6, 16),
			substr((string) $recordData, 22, 16),
			substr((string) $recordData, 38, 16),
			$this->_md5Ctxt
		)) {
	 ...
	 * 		Excel File Format"
	 */
	private function _readCodepage(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readDateMode(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read a FONT record
	 */
	private function _readFont(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringShort(substr((string) $recordData, 14));
			} else {
				$string = $this->_readByteStringShort(substr((string) $recordData, 14));
			}
	 ...
	 * 		Excel File Format"
	 */
	private function _readFormat(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringLong(substr((string) $recordData, 2));
			} else {
				// BIFF7
				$string = $this->_readByteStringShort(substr((string) $recordData, 2));
			}
	 ...
	 *
	 */
	private function _readXfExt(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read STYLE record
	 */
	private function _readStyle(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read PALETTE record
	 */
	private function _readPalette(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readSheet(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if ($this->_version == self::XLS_BIFF8) {
			$string = self::_readUnicodeStringShort(substr((string) $recordData, 6));
			$rec_name = $string['value'];
		} elseif ($this->_version == self::XLS_BIFF7) {
			$string = $this->_readByteStringShort(substr((string) $recordData, 6));
			$rec_name = $string['value'];
		}
	 ...
	 */
	private function _readExternalBook(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (strlen((string) $recordData) > 4) {
			...
			$encodedUrlString = self::_readUnicodeStringLong(substr((string) $recordData, 2));
			$offset += $encodedUrlString['size'];

			// offset: var; size: var; list of $nm sheet names (Unicode strings, 16-bit length)
			$externalSheetNames = [];
			for ($i = 0; $i < $nm; ++$i) {
				$externalSheetNameString = self::_readUnicodeStringLong(substr((string) $recordData, $offset));
	 ...
	 */
	private function _readExternName(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			// offset: 6; size: var
			$nameString = self::_readUnicodeStringShort(substr((string) $recordData, 6));

			// offset: var; size: var; formula data
			$offset = 6 + $nameString['size'];
			$formula = $this->_getFormulaFromStructure(substr((string) $recordData, $offset));
	 ...
	 */
	private function _readExternSheet(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readDefinedName(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$string = self::_readUnicodeString(substr((string) $recordData, 14), $nlen);
	 ...
	 */
	private function _readMsoDrawingGroup(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
	 ...
	 * 		Excel File Format"
	 **/
	private function _readSst(): void {
	 ...
	 */
	private function _readPrintGridlines(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readDefaultRowHeight(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read SHEETPR record
	 */
	private function _readSheetPr(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readHorizontalPageBreaks(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readVerticalPageBreaks(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read FOOTER record
	 */
	private function _readFooter(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read HCENTER record
	 */
	private function _readHcenter(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read VCENTER record
	 */
	private function _readVcenter(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readLeftMargin(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readRightMargin(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readTopMargin(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readBottomMargin(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readPageSetup(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (!$this->_readDataOnly) {
			...
			$marginHeader = self::_extractNumber(substr((string) $recordData, 16, 8));
			$this->_phpSheet->getPageMargins()->setHeader($marginHeader);

			// offset: 24; size: 8; footer margin (IEEE 754 floating-point value)
			$marginFooter = self::_extractNumber(substr((string) $recordData, 24, 8));
	 ...
	 */
	private function _readProtect(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * SCENPROTECT
	 */
	private function _readScenProtect(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * OBJECTPROTECT
	 */
	private function _readObjectProtect(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readPassword(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readDefColWidth(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read COLINFO record
	 */
	private function _readColInfo(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readRow(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readLabelSst(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readMulRk(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readNumber(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
			...
			$numValue = self::_extractNumber(substr((string) $recordData, 6, 8));
	 ...
	 * 		Excel File Format"
	 */
	private function _readFormula(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
			...
			if ((ord($recordData[6]) == 0) && (ord($recordData[12]) == 255) && (ord($recordData[13]) == 255)) {
				...
				$value = self::_extractNumber(substr((string) $recordData, 6, 8));
			}
	 ...
	 */
	private function _readSharedFmla(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readBoolErr(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readMulBlank(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readLabel(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringLong(substr((string) $recordData, 6));
				$value = $string['value'];
			} else {
				$string = $this->_readByteStringLong(substr((string) $recordData, 6));
				$value = $string['value'];
			}
			$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
	 ...
	 * Read BLANK record
	 */
	private function _readBlank(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readMsoDrawing(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
	 ...
	 * Read OBJ record
	 */
	private function _readObj(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read WINDOW2 record
	 */
	private function _readWindow2(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readPageLayoutView(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read SCL record
	 */
	private function _readScl(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * Read PANE record
	 */
	private function _readPane(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readSelection(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 * 		Excel File Format"
	 */
	private function _readMergedCells(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
				if ((str_contains($cellRangeAddress, ':')) && ($this->_includeCellRangeFiltered($cellRangeAddress))) {
	 ...
	 */
	private function _readHyperLink(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (!$this->_readDataOnly) {
			...
			if ($hasDesc) {
				...
				$desc = self::_encodeUTF16(substr((string) $recordData, 36, 2 * ($dl - 1)), false);
				$offset += 4 + 2 * $dl;
			}
			...
			switch ($hyperlinkType) {
				case 'URL':
					...
					$url = self::_encodeUTF16(substr((string) $recordData, $offset, $us - 2), false);
			...
			if ($hasText) {
				...
				$text = self::_encodeUTF16(substr((string) $recordData, $offset, 2 * ($tl - 1)), false);
				$url .= $text;
			}
	 ...
	 */
	private function _readDataValidations(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readDataValidation(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		$offset = 4;
		$string = self::_readUnicodeStringLong(substr((string) $recordData, $offset));
		...
		$cellRangeAddressList = $this->_readBIFF8CellRangeAddressList(substr((string) $recordData, $offset));
	 ...
	 */
	private function _readSheetLayout(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readSheetProtection(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	 ...
	 */
	private function _readRangeProtection(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
		...
		if (!$this->_readDataOnly) {
			...
			for ($i = 0; $i < $cref; ++$i) {
				try {
					$cellRange = $this->_readBIFF8CellRangeAddressFixed(substr((string) $recordData, 27 + 8 * $i, 8));
				} catch (PHPExcel_Exception) {
					return;
				}
	 ...
	 * Read IMDATA record
	 */
	private function _readImData(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
	 ...
	 */
	private function _readContinue(): void {
		$length = self::_GetInt2d($this->_data, $this->_pos + 2);
		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	...
	private function _getFormulaFromData($formulaData, $additionalData = '', $baseCell = 'A1') {
		...
		while (strlen((string) $formulaData) > 0 and $token = $this->_getNextToken($formulaData, $baseCell)) {
			$tokens[] = $token;
	...
	private function _readBIFF8CellRangeAddressList($subData) {
		...
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF8CellRangeAddressFixed(substr((string) $subData, $offset, 8));
			$offset += 8;
		}
	...
	private function _readBIFF5CellRangeAddressList($subData) {
		...
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF5CellRangeAddressFixed(substr((string) $subData, $offset, 6));
			$offset += 6;
		}
	...
	private static function _readBIFF8Constant($valueData) {
		...
		switch ($identifier) {
			...
			case 0x01: // number
				...
				$value = self::_extractNumber(substr((string) $valueData, 1, 8));
				$size = 9;
				break;
			case 0x02: // string value
				// offset: 1; size: var; Unicode string, 16-bit string length
				$string = self::_readUnicodeStringLong(substr((string) $valueData, 1));
				$value = '"' . $string['value'] . '"';
				$size = 1 + $string['size'];
				break;
			case 0x04: // boolean
	...
	private function _readByteStringShort($subData) {
		...
		$value = $this->_decodeCodepage(substr((string) $subData, 1, $ln));
	...
	private function _readByteStringLong($subData) {
		...
		$value = $this->_decodeCodepage(substr((string) $subData, 2));
	...
	private static function _readUnicodeStringShort($subData) {
		...
		$string = self::_readUnicodeString(substr((string) $subData, 1), $characterCount);
	...
	private static function _readUnicodeStringLong($subData) {
		...
		$string = self::_readUnicodeString(substr((string) $subData, 2), $characterCount);
	...
	private static function _readUnicodeString($subData, $characterCount) {
		...
		// needs to be fixed
		$value = self::_encodeUTF16(substr((string) $subData, 1, $isCompressed ? $characterCount : 2 * $characterCount), $isCompressed);
