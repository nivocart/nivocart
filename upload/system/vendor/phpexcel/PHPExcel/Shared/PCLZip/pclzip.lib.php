...
class PclZip {
			...
			if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] !== 0)) {
				...
				for ($j = 0; ($j < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
					...
					if (str_ends_with((string) $p_options[PCLZIP_OPT_BY_NAME][$j], "/")) {
						...
						if ((strlen($v_header['stored_filename']) > strlen((string) $p_options[PCLZIP_OPT_BY_NAME][$j])) && (str_starts_with($v_header['stored_filename'], (string) $p_options[PCLZIP_OPT_BY_NAME][$j]))) {
							$v_extract = true;
						}
	...
	public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options) {
		...
		if ($p_entry['status'] === 'ok') {
			...
			if (file_exists($p_entry['filename'])) {
				...
				if (is_dir($p_entry['filename'])) {
					...
					// ----- Look for PCLZIP_OPT_STOP_ON_ERROR
					// For historical reason first PclZip implementation does not stop when this kind of error occurs.
					if ((isset($p_options[PCLZIP_OPT_STOP_ON_ERROR])) && ($p_options[PCLZIP_OPT_STOP_ON_ERROR] === true)) {
			...
			for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
				...
				if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] !== 0)) {
					...
					for ($j = 0; ($j < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
						...
						if (str_ends_with((string) $p_options[PCLZIP_OPT_BY_NAME][$j], "/")) {
							// ----- Look if the directory is in the filename path
							if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen((string) $p_options[PCLZIP_OPT_BY_NAME][$j])) && (str_starts_with($v_header_list[$v_nb_extracted]['stored_filename'], (string) $p_options[PCLZIP_OPT_BY_NAME][$j]))) {
								$v_found = true;
		...
		// Parameters :
		// --------------------------------------------------------------------------------
		function privErrorLog($p_error_code = 0, $p_error_string = ''): void {
			if (PCLZIP_ERROR_EXTERNAL === 1) {
