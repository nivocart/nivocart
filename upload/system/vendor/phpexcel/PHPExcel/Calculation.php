...
class PHPExcel_Calculation {
	 ...
	 * @var array
	 */
	private static $_binaryOperators = [
		'+' => true, '-' => true, '*' => true, '/' => true,
		'^' => true, '&' => true, '>' => true, '<' => true,
		'=' => true, '>=' => true, '<=' => true, '<>' => true,
		'|' => true, ':' => true,
	 ...
	 *
	 */
	private static $_validLocaleLanguages = ['en'];
	 ...
	 *
	 */
	private static $_ExcelConstants = [
		'TRUE' => true,
		'FALSE' => false,
		'NULL' => null,
	];
	...
	private static function _loadLocales(): void {
	 ...
	 */
	public static function unsetInstance(PHPExcel $workbook = null): void {
		if ($workbook !== null) {
	 ...
	 */
	private static function _resizeMatricesShrink(&$matrix1, &$matrix2, $matrix1Rows, $matrix1Columns, $matrix2Rows, $matrix2Columns): void {
	 ...
	 * @param	integer		$matrix2Columns	Column size of second matrix operand
	 */
	private static function _resizeMatricesExtend(&$matrix1, &$matrix2, $matrix1Rows, $matrix1Columns, $matrix2Rows, $matrix2Columns): void {
		if (($matrix2Columns < $matrix1Columns) || ($matrix2Rows < $matrix1Rows)) {
			if ($matrix2Columns < $matrix1Columns) {
				for ($i = 0; $i < $matrix2Rows; ++$i) {
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = null) {
		if (($formula = $this->_convertMatrixReferences(trim((string) $formula))) === false) {
			return false;
		}
		...
		while (true) {
			...
			if ($opCharacter == '-' && !$expectingOperator) {				//	Is it a negation instead of a minus?
				...
				if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $val, $matches)) {
					...
					if ($testPrevOp['value'] == ':') {
						...
						} elseif ((ctype_alpha((string) $startRowColRef)) && (ctype_alpha($val)) && (strlen((string) $startRowColRef) <= 3) && (strlen($val) <= 3)) {
							//	Column range
			...
			if ($formula[$index] == ' ') {
				...
				if (($expectingOperator) && (preg_match('/^' . self::CALCULATION_REGEXP_CELLREF . '.*/Ui', substr((string) $formula, $index), $match)) && ($output[count($output) - 1]['type'] == 'Cell Reference')) {
