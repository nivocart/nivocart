...
class Cpdf {
	 ...
	 * @param $font
	 */
	private function openFont($font): void {
	 ...
	 * @param bool $force
	 */
	public function setColor($color, $force = false): void {
	 ...
	 * @param bool $force
	 */
	public function setStrokeColor($color, $force = false): void {
		$new_color = [$color[0], $color[1], $color[2], $color[3] ?? null];
	 ...
	 */
	public function setLineTransparency($mode, $opacity): void {
		static $blend_modes = [
			"Normal",
			"Multiply",
			"Screen",
	 ...
	 * @param float  $opacity 0.0 fully transparent, 1.0 fully opaque
	 */
	public function setFillTransparency($mode, $opacity): void {
		static $blend_modes = [
			"Normal",
			"Multiply",
			"Screen",
	 ...
	 * @param bool $f
	 */
	public function polygon($p, $np, $f = false): void {
		$this->addContent(sprintf("\n%.3F %.3F m ", $p[0], $p[1]));
	 ...
	 * @throws Exception
	 */
	public function addImagePng($file, &$img, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null): void {
	 ...
	 * @param $byte
	 */
	protected function addImagePngAlpha($file, $x, $y, $w, $h, $byte): void {
		// generate images
			...
			for ($xpx = 0; $xpx < $wpx; ++$xpx) {
				for ($ypx = 0; $ypx < $hpx; ++$ypx) {
					...
					if ($eight_bit) {
						...
						$gammacorr = 2.2;
						$pixel = (((127 - $alpha) * 255 / 127) / 255) ** $gammacorr * 255;
					} else {
	 ...
	 * @param null $mask
	 */
	public function addPngFromBuf($file, &$data, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null): void {
		if (isset($this->imagelist[$file])) {
			$data = null;
	 ...
	 * @param string $code
	 */
	public function addJavascript($code): void {
