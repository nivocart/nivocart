...
class File extends BinaryStream {
	...
	public function setTableOffset($offset): void {
	...
	public function setSubset($subset): void {
		if (!is_array($subset)) {
	...
	public function encode($tags = []): void {
		if (!self::$raw) {
	...
	public function parseTableEntries(): void {
		$this->parseHeader();
	...
	protected function readTable($tag): void {
		$this->parseTableEntries();
	...
	public function setTableObject($name, Table $data): void {
		$this->data[$name] = $data;
	}
	...
	public function addDirectoryEntry(DirectoryEntry $entry): void {
		$this->directory[$entry->tag] = $entry;
	}

	public function saveAdobeFontMetrics($file, $encoding = null): void {
