...
class OutlineSimple extends Outline {
	...
	public function encodePoints($points) {
		...
		$l += $font->writeFWord($this->xMin ?? $xMin); // xMin
		$l += $font->writeFWord($this->yMin ?? $yMin); // yMin
		$l += $font->writeFWord($this->xMax ?? $xMax); // xMax
		$l += $font->writeFWord($this->yMax ?? $yMax); // yMax
