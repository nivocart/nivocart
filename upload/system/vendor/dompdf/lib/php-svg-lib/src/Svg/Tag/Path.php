...
class Path extends Shape {
	...
	public static $repeatedCommands = [
		'm' => 'l',
		'M' => 'L',
	];
		...
		foreach ($commands as $c) {
			if (count($c) == 3) {
				...
				if (isset(self::$commandLengths[$commandLower]) && ($commandLength = self::$commandLengths[$commandLower]) && count($item) > $commandLength) {
					$repeatedCommand = self::$repeatedCommands[$c[1]] ?? $c[1];
					$command = $c[1];
	...
	public function drawArc(SurfaceInterface $surface, $fx, $fy, $coords): void {
		$rx = $coords[0];
		$ry = $coords[1];
		$rot = $coords[2];
		$large = $coords[3];
	...
	public function arcToSegments($toX, $toY, $rx, $ry, $large, $sweep, $rotateX) {
		$th = $rotateX * M_PI / 180;
		$sinTh = sin($th);
		$cosTh = cos($th);
		$fromX = 0;
	...
	public function segmentToBezier($th2, $th3, $cosTh, $sinTh, $rx, $ry, $cx1, $cy1, $mT, $fromX, $fromY) {
		$costh2 = cos($th2);
		$sinth2 = sin($th2);
		$costh3 = cos($th3);
		$sinth3 = sin($th3);
