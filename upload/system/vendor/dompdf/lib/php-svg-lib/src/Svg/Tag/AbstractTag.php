...
abstract class AbstractTag {
	...
	protected function start($attributes): void {
	}
	...
	protected function setStyle(Style $style): void {
		$this->style = $style;
	...
	protected function applyTransform($attributes): void {
		if (isset($attributes["transform"])) {
			...
			foreach ($transformations as $t) {
				switch ($t[0]) {
					...
					case "translate":
						$surface->translate($t[1], $t[2] ?? 0);
						break;

					case "scale":
						$surface->scale($t[1], $t[2] ?? $t[1]);
						break;
