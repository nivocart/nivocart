...
class Block extends AbstractFrameReflower {
	 ...
	 */
	protected function _text_align(): void {
	...
	public function reflow(BlockFrameDecorator $block = null) {
		...
		$top = (float)$style->length_in_pt([$style->margin_top, $style->padding_top, $style->border_top_width], $cb["h"]);
		$bottom = (float)$style->length_in_pt([$style->border_bottom_width, $style->margin_bottom, $style->padding_bottom], $cb["h"]);

		$cb_x = $x + (float)$left_margin + (float)$style->length_in_pt([$style->border_left_width, $style->padding_left], $cb["w"]);
