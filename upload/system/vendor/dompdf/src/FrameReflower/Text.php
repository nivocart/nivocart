	...
	public function get_min_max_width() {
		...
		switch ($style->white_space) {
			...
			case "pre-line":
				...
				array_walk($words, function (&$val, $str) use ($font, $size, $word_spacing, $char_spacing, $root): void {
			...
			case "pre":
				...
				array_walk($lines, function (&$val, $str) use ($font, $size, $word_spacing, $char_spacing, $root): void {
					$val = $root->getFontMetrics()->getTextWidth($str, $font, $size, $word_spacing, $char_spacing);
				});
		...
		$delta = (float)$style->length_in_pt([$style->margin_left, $style->border_left_width, $style->padding_left, $style->padding_right, $style->border_right_width, $style->margin_right], $line_width);
