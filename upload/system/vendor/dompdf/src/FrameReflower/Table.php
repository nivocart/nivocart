...
class Table extends AbstractFrameReflower {
	...
	protected function _assign_widths() {
		...
		if (!$centered) {
			$delta += (float)$style->length_in_pt([$style->padding_left, $style->border_left_width, $style->border_right_width, $style->padding_right], $cb["w"]);
		}
	...
	public function reflow(BlockFrameDecorator $block = null) {
		...
		$content_x = $x + (float)$left + (float)$style->length_in_pt([$style->padding_left, $style->border_left_width], $cb["w"]);
		$content_y = $y + (float)$style->length_in_pt([$style->margin_top, $style->border_top_width, $style->padding_top], $cb["h"]);
	...
	public function get_min_max_width() {
		...
		$dims = [$style->border_left_width, $style->border_right_width, $style->padding_left, $style->padding_right, $style->margin_left, $style->margin_right];
