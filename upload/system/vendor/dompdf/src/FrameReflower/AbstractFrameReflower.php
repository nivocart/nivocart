...
abstract class AbstractFrameReflower {
	...
	public function get_min_max_width() {
		...
		for ($iter = $this->_frame->get_children()->getIterator(); $iter->valid(); $iter->next()) {
			...
			if ($iter->valid()) {
				[$low[], $high[]] = $iter->current()->get_min_max_width();
				continue;
			}
		}
	...
	protected function _parse_string($string, $single_trim = false) {
		...
		$string = preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/", fn ($matches) => \Dompdf\Helpers::unichr(hexdec($matches[1])), $string);
	...
	protected function _set_content(): void {
		...
		if ($style->counter_reset && ($reset = $style->counter_reset) !== "none") {
			...
			$frame->reset_counter($vars[0], ($frame->_counters['__' . $vars[0]] ?? ($vars[1] ?? 0)));
		}
