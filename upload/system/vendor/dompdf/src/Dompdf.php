...
class Dompdf {
	...
	*/
	 ...
	 */
	private function saveLocale(): void {
	 ...
	 * @throws Exception
	 */
	public function loadHtmlFile($file): void {
		$this->saveLocale();
		...
		if ($protocol == "" || $protocol === "file://") {
			...
			if ($chroot && !str_starts_with($realfile, $chroot)) {
	 ...
	 * @deprecated
	 */
	public function load_html($str, $encoding = 'UTF-8'): void {
	 ...
	 */
	public function loadHtml($str, $encoding = 'UTF-8'): void {
		$this->saveLocale();
		...
		} elseif (isset($document_encoding) === false && str_contains($str, '<head>')) {
	 ...
	 * @param string $cacheId
	 */
	public function enableCaching($cacheId): void {
		...
		if ($options->isFontSubsettingEnabled() && $canvas instanceof CPDF) {
			foreach ($this->tree->get_frames() as $frame) {
				...
				if ($frame->get_node()->nodeName == "dompdf_generated") {
					...
					$decoded_string = preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/", fn ($matches) => \Dompdf\Helpers::unichr(hexdec($matches[1])), $style->content);
	 ...
	 * @return void
	 */
	private function write_log(): void {
	 ...
	 * @deprecated
	 */
	public function set_paper($size, $orientation = "portrait"): void {
	 ...
	 * @deprecated
	 */
	public function set_callbacks($callbacks): void {
