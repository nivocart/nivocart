...
class Stylesheet {
	...
	public function __construct(Dompdf $dompdf) {
		...
		[$this->_protocol, $this->_base_host, $this->_base_path] = Helpers::explode_url($_SERVER["SCRIPT_FILENAME"]);
		$this->_page_styles = ["base" => null];
	}
	 ...
	 */
	public function set_protocol($protocol): void {
	 ...
	 */
	public function add_style($key, Style $style): void {
		if (!is_string($key)) {
	...
	public function apply_styles(FrameTree $tree): void {
		...
		foreach ($this->_styles as $selector => $selector_styles) {
			foreach ($selector_styles as $style) {
				if (!str_contains($selector, ":before") && !str_contains($selector, ":after")) {
					continue;
				}
				...
				foreach ($applied_styles as $arr) {
					foreach ($arr as $s) {
						...
						foreach ($media_queries as $media_query) {
							[$media_query_feature, $media_query_value] = $media_query;
	 ...
	 * @throws Exception
	 */
	private function _parse_css($str): void {
		$str = trim($str);
	 ...
	 */
	private function _parse_import($url): void {
	 ...
	 */
	private function _parse_font_face($str): void {
	 ...
	 */
	private function _parse_sections($str, $media_queries = []): void {
