...
class GD implements Canvas {
	 ...
	 */
	public function __construct($size, $orientation, Dompdf $dompdf, $aa_factor = 1.0, $bg_color = [1, 1, 1, 0]) {
		if (!is_array($size)) {
	...
	public function page_text(): void {
		// Not implemented
	}
	 ...
	 *     'page' => Number of the page to output (defaults to the first).
	 */
	private function _output($options = []): void {
		if (!isset($options["type"])) {
			$options["type"] = "png";
		}
