...
class PDFLib implements Canvas {
	 ...
	 * @param Dompdf $dompdf
	 */
	public function __construct($paper, $orientation, Dompdf $dompdf) {
		if (is_array($paper)) {
			$size = $paper;
	 ...
	 */
	public function add_info($label, $value): void {
	 ...
	 */
	protected function _place_objects(): void {
	 ...
	 * @return void
	 */
	protected function _set_line_style($width, $cap, $join, $dash): void {
		if (count($dash) == 1) {
			$dash[] = $dash[0];
		}
	 ...
	 */
	protected function _set_stroke_color($color): void {
	 ...
	 * @param array $color array(r,g,b)
	 */
	protected function _set_fill_color($color): void {
	 ...
	 * @param $mode
	 */
	public function set_opacity($opacity, $mode = "Normal"): void {
		if ($mode === "Normal") {
	 ...
	 * @param array $style
	 */
	public function line($x1, $y1, $x2, $y2, $color, $width, $style = null): void {
	 ...
	 * @param array $style
	 */
	public function arc($x1, $y1, $r1, $r2, $astart, $aend, $color, $width, $style = []): void {
		$this->_set_line_style($width, "butt", "", $style);
		$this->_set_stroke_color($color);
	 ...
	 * @param null $style
	 */
	public function rectangle($x1, $y1, $w, $h, $color, $width, $style = null): void {
		$this->_set_stroke_color($color);
		$this->_set_line_style($width, "butt", "", $style);
	 ...
	 * @param bool $fill
	 */
	public function polygon($points, $color, $width = null, $style = null, $fill = false): void {
		$this->_set_fill_color($color);
		$this->_set_stroke_color($color);
	 ...
	 * @param bool $fill
	 */
	public function circle($x, $y, $r, $color, $width = null, $style = null, $fill = false): void {
		$this->_set_fill_color($color);
		$this->_set_stroke_color($color);
	 ...
	 */
	public function image($img_url, $x, $y, $w, $h, $resolution = "normal"): void {
		$w = (int)$w;
		$h = (int)$h;
	 ...
	 */
	public function page_script($code, $type = "text/php"): void {
		$_t = "script";
	 ...
	 */
	protected function _add_page_text(): void {
		...
		for ($p = 1; $p <= $this->_page_count; $p++) {
			...
			foreach ($this->_page_text as $pt) {
				...
				switch ($_t) {
					case "text":
						$text = str_replace(["{PAGE_NUM}", "{PAGE_COUNT}"], [$p, $this->_page_count], $text);
