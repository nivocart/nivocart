...
class ModelPaymentGlobalpay extends Model {
	...
	public function addTransaction(int $globalpay_order_id, $type, $order_info): void {
		$this->db->query("INSERT INTO " . DB_PREFIX . "globalpay_order_transaction SET globalpay_order_id = '" . (int)$globalpay_order_id . "', date_added = NOW(), `type` = '" . $this->db->escape($type) . "', amount = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false, $this->config->get('config_currency')) . "'");
	}

	public function addHistory($order_id, $order_status_id, $comment): void {
