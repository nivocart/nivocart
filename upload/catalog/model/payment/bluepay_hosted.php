...
class ModelPaymentBluepayHosted extends Model {
	...
	public function addTransaction($bluepay_hosted_order_id, $type, $order_info): void {
		$this->db->query("INSERT INTO " . DB_PREFIX . "bluepay_hosted_order_transaction SET bluepay_hosted_order_id = '" . (int)$bluepay_hosted_order_id . "', date_added = NOW(), `type` = '" . $this->db->escape($type) . "', amount = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false, $this->config->get('config_currency')) . "'");
	}
