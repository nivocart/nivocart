<?php
/**
 * Class ControllerCommonContentLow
 *
 * @package NivoCart
 */
class ControllerCommonContentLow extends Controller {
	...
	protected function index() {
		...
		foreach ($extensions as $extension) {
			...
			if ($modules) {
				foreach ($modules as $module) {
					if ($module['layout_id'] == $layout_id && $module['position'] == 'content_low' && $module['status']) {
						$module_data[] = array(
							'code'       => $extension['code'],
							'setting'    => $module,
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/content_low.tpl')) {
			...
			$this->template = 'default/template/common/content_low.tpl';
		}
