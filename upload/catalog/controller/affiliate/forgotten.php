<?php
/**
 * Class ControllerAffiliateForgotten
 *
...
class ControllerAffiliateForgotten extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] === 'POST') && $this->validateEmail()) {
			...
			$this->model_affiliate_affiliate->saveResetToken($this->request->post['email'], $hashed_token, $expires_at);

			// 3. Build the reset URL containing the raw token.
			$reset_url = $this->url->link('affiliate/forgotten/reset', 'token=' . $raw_token . '&email=' . $this->request->post['email'], 'SSL');
		...
		$this->renderTemplate('affiliate/forgotten');
	}
	...
	public function reset() {
		...
		if (!$this->validateToken($raw_token, $email)) {
			...
			$this->redirect($this->url->link('affiliate/forgotten', '', 'SSL'));
		}
		...
		$this->data['action'] = $this->url->link('affiliate/forgotten/confirm', '', 'SSL');

		$this->data['token'] = $raw_token;
		$this->data['email'] = $email;

		$this->renderTemplate('affiliate/forgotten_reset');
	}
	...
	public function confirm() {
		...
		$this->model_affiliate_affiliate->editPassword($email, $this->request->post['password']);

		// Immediately invalidate the token so it cannot be reused.
		$this->model_affiliate_affiliate->clearResetToken($email);
	...
	protected function validateEmail() {
		...
		$email_exists = (bool) $this->model_affiliate_affiliate->getTotalAffiliatesByEmail($email);
	...
	private function validateToken(string $raw_token, string $email): bool {
		...
		$record = $this->model_affiliate_affiliate->getResetToken($email);
