 ...
 * @package NivoCart
 *
 * Standard checkout confirm page.
 * Renders the payment widget inline via getChild() — Stripe's index() fires here,
 * creating the PaymentIntent and rendering the card widget. send() handles
 * completion via AJAX after Stripe.js confirms the card in the browser.
 *
 * Interactive gateways (Stripe etc.) render their own card widget via index().
 * Silent gateways (free checkout, COD etc.) use getChild() as before.
 */
class ControllerCheckoutConfirm extends Controller {
    /**
     * Gateways that collect and confirm payment in the browser.
     * getChild() still fires for these (it calls index(), rendering the widget)
     * but the guard below prevents a fatal if the gateway isn't properly
     * configured. Add new interactive gateways here as they are built.
     */
    ...
    public function index() {
        ...
        if (!$redirect) {
            ...
            // --- Payment widget ---
            // For interactive gateways: getChild() calls index(), which creates the
            // PaymentIntent and renders the card widget. send() completes via AJAX.
            // For silent gateways: getChild() calls index() as before — no change.
            $payment_code = $this->session->data['payment_method']['code'];

            $this->data['payment'] = $this->getChild('payment/' . $payment_code);
