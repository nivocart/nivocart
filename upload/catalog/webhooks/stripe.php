...
// -------------------------------------------------------------------------
$result = $db->query("SELECT `key`, `value` FROM `" . DB_PREFIX . "setting` WHERE `group` = 'stripe_payments'");
...
$failed_status = (int)($settings['stripe_payments_order_failed_id'] ?? 10);
$disputed_status = (int)($settings['stripe_payments_order_disputed_id'] ?? 13);
...
switch ($event_type) {
    ...
    case 'payment_intent.payment_failed':
        handlePaymentFailed($object, $db, $log, $settings);
        break;

    case 'charge.dispute.created':
        handleDisputeCreated($object, $db, $log, $settings);
        break;
 ...
 */
function handlePaymentFailed(array $intent, object $db, object $log, array $settings): void {
    $order_id = (int)($intent['metadata']['order_ref'] ?? 0);
	...
	$failed_status_id = (int)($settings['stripe_payments_order_failed_id'] ?? 10);
 ...
 */
function handleDisputeCreated(array $charge, object $db, object $log, array $settings): void {
	...
	$disputed_status_id = (int)($settings['stripe_payments_order_disputed_id'] ?? 13);
